...
router.post('/posts', auth.requireMember, (req, res) => {
...
router.get('/posts/:id', (req, res) => {
  const post = db.prepare(`${POST_SELECT} WHERE p.id = ?`).get(Number(req.params.id));
  if (!post) return res.status(404).render('404', { title: 'Post not found' });

  const author = db
    .prepare('SELECT id, name, apartment, bio, email, phone FROM users WHERE id = ?')
    .get(post.user_id);
  const acceptor = post.accepted_by
    ? db.prepare('SELECT id, name, apartment FROM users WHERE id = ?').get(post.accepted_by)
    : null;
...
router.get('/posts/:id/edit', auth.requireMember, (req, res) => {
...
router.post('/posts/:id/edit', auth.requireMember, (req, res) => {
  const post = db.prepare('SELECT * FROM posts WHERE id = ?').get(Number(req.params.id));
  if (!post || post.user_id !== req.user.id)
    return res.status(404).render('404', { title: 'Post not found' });
...
router.post('/posts/:id/accept', auth.requireMember, (req, res) => {
  const post = db.prepare('SELECT * FROM posts WHERE id = ?').get(Number(req.params.id));
  if (!post) return res.status(404).render('404', { title: 'Post not found' });
  ...
  const claimed = db
    .prepare(
      `UPDATE posts SET status = 'matched', accepted_by = ?, accepted_at = datetime('now')
  ...
  const convo = findOrCreateConversation(post.id, post.user_id, req.user.id);
  db.prepare(
...
router.post('/posts/:id/ask', auth.requireMember, (req, res) => {
  const post = db.prepare('SELECT * FROM posts WHERE id = ?').get(Number(req.params.id));
  if (!post) return res.status(404).render('404', { title: 'Post not found' });
  if (post.user_id === req.user.id) return res.redirect(`/posts/${post.id}`);
...
router.post('/posts/:id/status', auth.requireMember, (req, res) => {
  const post = db.prepare('SELECT * FROM posts WHERE id = ?').get(Number(req.params.id));
  if (!post) return res.status(404).render('404', { title: 'Post not found' });
  ...
  if (action === 'complete' && post.status === 'matched') {
    db.prepare("UPDATE posts SET status = 'completed' WHERE id = ?").run(post.id);
...
router.get('/my-posts', auth.requireMember, (req, res) => {
  const mine = db
    .prepare(`${POST_SELECT} WHERE p.user_id = ? ORDER BY p.created_at DESC`)
    .all(req.user.id);
  const accepted = db
    .prepare(`${POST_SELECT} WHERE p.accepted_by = ? ORDER BY p.accepted_at DESC`)
    .all(req.user.id);
